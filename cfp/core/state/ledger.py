...
class Ledger:
    ...
    def __init__(
        ...
        # State tracking
        self.block_height = 0
        # Once genesis minting has happened, no further input-less (mint) txs.
        self.genesis_done = False
    ...
    def validate_transaction(
        ...
        if len(tx.inputs) == 0:
            # Mint is allowed once, at genesis.
            if self.genesis_done:
                return False, "Mint transactions only allowed at genesis"
            return True, ""

        # Reject duplicate inputs within a tx (would let the balance check count
        # a UTXO more than once while apply removes it only once).
        seen_utxo_ids: Set[bytes] = set()
        seen_nullifiers: Set[bytes] = set()
        for i, inp in enumerate(tx.inputs):
            if inp.utxo_id in seen_utxo_ids:
                return False, f"Input {i}: duplicate UTXO reference within transaction"
            if inp.nullifier in seen_nullifiers:
                return False, f"Input {i}: duplicate nullifier within transaction"
            seen_utxo_ids.add(inp.utxo_id)
            seen_nullifiers.add(inp.nullifier)
        ...
        if check_signatures:
            from cfp.crypto import keccak256, recover_public_key
    ...
    def apply_transaction(
        ...
        # SECURITY: a successfully applied input-less tx is the genesis mint;
        # close the mint path for everything after it.
        if len(tx.inputs) == 0:
    ...
    def _create_state_snapshot(self) -> dict:
        ...
        return {
            ...
            'genesis_done': self.genesis_done,
    ...
    def _restore_state_snapshot(self, snapshot: dict) -> None:
        ...
        self.genesis_done = snapshot.get('genesis_done', self.block_height > 0)
    ...
    def _load_from_storage(self) -> None:
        ...
        # SECURITY: any persisted state means genesis has already happened.
        self.genesis_done = bool(self.utxo_set) or bool(self.nullifier_set) or self.block_height > 0
