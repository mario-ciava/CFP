...
from cfp.crypto import bytes_to_hex, sha256, sign, verify
...
class Transaction:
    ...
    def validate_structure(self) -> tuple[bool, str]:
        ...
        # SECURITY: input/output counts are serialized as a single byte, so more
        # than 255 of either would silently corrupt (or raise) on serialize.
        if len(self.inputs) > 255:
            return False, f"Too many inputs: {len(self.inputs)} > 255"
        if len(self.outputs) > 255:
            return False, f"Too many outputs: {len(self.outputs)} > 255"
        ...
        for i, inp in enumerate(self.inputs):
            ...
            # SECURITY: output_index is serialized as a single byte.
            if not (0 <= inp.output_index <= 255):
                return False, f"Input {i}: output_index must be 0-255, got {inp.output_index}"
