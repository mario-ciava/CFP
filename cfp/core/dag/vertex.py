...
class PayloadType(IntEnum):
    ...
    - BLOCK: Carries a produced block (hash + state root) into the DAG
    """
    TRANSACTION = 0
    INTENT = 1
    CHECKPOINT = 2
...
class Vertex:
    ...
    def validate_structure(self) -> tuple[bool, str]:
        ...
        # vertex_id and signature are mandatory: an unsigned vertex is invalid.
        if not self.verify_vertex_id():
            return False, "Vertex ID missing or does not match content hash"

        if not self.verify_signature():
            return False, "Signature missing or invalid"
