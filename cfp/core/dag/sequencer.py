...
from cfp.core.dag.vertex import Vertex
...
class DAGSequencer:
    ...
    def _process_orphans(self, arrived_id: bytes) -> None:
        """
        Insert orphans that become ready once ``arrived_id`` (and any vertex
        promoted from the pool) is present.

        Uses an explicit work queue so a deep reverse-ordered chain is processed
        iteratively: O(N) time, O(1) stack depth. Orphans were structurally
        validated when queued; here we re-check membership, remaining parents,
        and timestamp order.
        """
        queue: deque = deque([arrived_id])
        while queue:
            current = queue.popleft()
            for orphan in self.orphan_pool.get_ready(current):
                # Already inserted via some other path.
                if orphan.vertex_id in self.vertices:
                    self.orphan_pool.remove(orphan.vertex_id)
                    continue
                # Still waiting on other parents.
                if any(p not in self.vertices for p in orphan.parents):
                    continue
                self.orphan_pool.remove(orphan.vertex_id)
                # Enforce timestamp ordering vs parents.
                max_parent_ts = max(self.vertices[p].timestamp for p in orphan.parents)
                if orphan.timestamp < max_parent_ts:
                    logger.warning(
                        f"Dropping orphan {orphan.vertex_id.hex()[:8]}...: "
                        f"timestamp {orphan.timestamp} < parent {max_parent_ts}"
                    )
                    continue
                self._insert_vertex(orphan)
                logger.info(f"Added vertex {orphan.vertex_id.hex()[:8]}... (promoted from orphan pool)")
                # Newly present vertex may unblock further orphans.
                queue.append(orphan.vertex_id)
    ...
    def has_vertex(self, vertex_id: bytes) -> bool:
        """Return True if the vertex is present in the DAG."""
    ...
    def linearize(self) -> List[bytes]:
        ...
        in_degree = dict.fromkeys(self.graph.nodes(), 0)
    ...
    def linearize_from(self, start_vertex: bytes) -> List[bytes]:
        ...
        in_degree = dict.fromkeys(subgraph.nodes(), 0)
