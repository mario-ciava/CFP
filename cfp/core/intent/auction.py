"""
Auction - Simplified (open, first-price) intent auction for CFP.

NOTE ON WHICH AUCTION IS CANONICAL
----------------------------------
This module is the **simplified** auction: bids are submitted in the clear and
the winner is chosen first-price. It is intentionally small and is used only by
the solver simulation and the unit tests.

The **canonical** CFP auction is the sealed-bid commit-reveal design with a
Poseidon transcript and ZK winner-selection proof, implemented by
``cfp.core.intent.verifiable_auction.VerifiableAuctionManager`` (this is what the
`cfp demo` command exercises). Prefer that one for anything protocol-facing.
...
from cfp.core.intent.intent import ExecutionTicket, Intent, SolverBid, TicketStatus
