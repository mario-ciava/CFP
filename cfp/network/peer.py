...
from cfp.network.protocol import MAGIC_BYTES, MAX_MESSAGE_SIZE, Message
...
class PeerInfo:
    ...
    last_seen: int = 0
    latency_ms: int = 0
    # Handshake state (see network/handshake.py):
    pubkey: Optional[bytes] = None       # peer's public key (once received)
    challenge: Optional[bytes] = None    # nonce we issued for the peer to sign
