...
class SyncManager:
    ...
    def _get_vertices_after(self, from_vertex_id: bytes) -> List[bytes]:
        ...
        # linearize() returns an ordered list of vertex IDs (bytes), not Vertex
        # objects — fetch each Vertex to serialize it.
        order = self.dag.linearize()

        if from_vertex_id == bytes(32):
            # From genesis: send everything (up to the batch cap).
            for vid in order:
                v = self.dag.get_vertex(vid)
                if v is None:
                    continue
            ...
            # Send only the vertices that come after from_vertex_id.
            found = False
            for vid in order:
                if found:
                    v = self.dag.get_vertex(vid)
