...
class MessageType(IntEnum):
    ...
    SYNC_RESPONSE = 6
    PEER_LIST = 7
    # Signed challenge-response handshake (mutual authentication).
    HELLO = 8          # initiator -> responder: pubkey + nonce
    HELLO_ACK = 9      # responder -> initiator: pubkey + nonce + sig(initiator nonce)
    HELLO_CONFIRM = 10  # initiator -> responder: sig(responder nonce)
...
# =============================================================================
# Handshake message framing (payloads only; crypto lives in handshake.py)
# =============================================================================


def create_hello(sender_id: bytes, pubkey: bytes, nonce: bytes) -> Message:
    """HELLO: initiator presents its public key and a fresh challenge nonce."""
    return Message(msg_type=MessageType.HELLO, payload=pubkey + nonce, sender_id=sender_id)


def parse_hello(payload: bytes) -> tuple:
    """Return (pubkey[64], nonce[32])."""
    if len(payload) != 96:
        raise ValueError("Invalid HELLO payload length")
    return payload[:64], payload[64:96]


def create_hello_ack(sender_id: bytes, pubkey: bytes, nonce: bytes, signature: bytes) -> Message:
    """HELLO_ACK: responder's key, its own nonce, and a signature over the initiator nonce."""
    return Message(
...
def parse_hello_ack(payload: bytes) -> tuple:
    """Return (pubkey[64], nonce[32], signature[64])."""
    if len(payload) != 160:
        raise ValueError("Invalid HELLO_ACK payload length")
    return payload[:64], payload[64:96], payload[96:160]


def create_hello_confirm(sender_id: bytes, signature: bytes) -> Message:
    """HELLO_CONFIRM: initiator's signature over the responder nonce."""
    return Message(msg_type=MessageType.HELLO_CONFIRM, payload=signature, sender_id=sender_id)


def parse_hello_confirm(payload: bytes) -> bytes:
    """Return signature[64]."""
    if len(payload) != 64:
        raise ValueError("Invalid HELLO_CONFIRM payload length")
    return payload
