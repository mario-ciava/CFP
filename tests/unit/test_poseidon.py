...
from cfp.crypto.poseidon import verify_params


class TestPoseidonCircomlibCompatibility:
    """
    Known-answer tests pinning the implementation to circomlib/snarkjs, so the
    Python hashes match what the Circom circuits compute.
    """

    # Canonical circomlib vectors (also produced by circomlibjs buildPoseidon()).
    CANONICAL = {
    ...
    @pytest.mark.parametrize("inputs,expected", list(CANONICAL.items()))
    def test_known_answer_vectors(self, inputs, expected):
        assert poseidon_hash(list(inputs)) == expected

    def test_verify_params_passes(self):
        # The shipped params must reproduce the known vectors.
        assert verify_params() is True

    def test_convenience_helpers_match(self):
        assert poseidon2(1, 2) == self.CANONICAL[(1, 2)]
