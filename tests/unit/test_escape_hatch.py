...
class TestEscapeHatchSelection:
    """Tests for the quota-aware transaction selection policy."""

    def test_low_priority_raw_is_reserved(self):
        """A low-fee raw tx must be kept even when higher-fee intents compete."""
        validator = EscapeHatchValidator(min_quota=0.10)
        # raw item has the lowest priority; intents dominate.
        raw = [("raw_low", 1)]
        intent = [(f"intent_{i}", 100 + i) for i in range(10)]
        selected = validator.select_transactions(raw, intent, max_count=4)
        assert "raw_low" in selected
        assert len(selected) == 4

    def test_no_quota_lets_raw_be_crowded_out(self):
        """With a 0% quota the low-fee raw tx is crowded out by fee priority."""
        validator = EscapeHatchValidator(min_quota=0.0)
        raw = [("raw_low", 1)]
        intent = [(f"intent_{i}", 100 + i) for i in range(10)]
        selected = validator.select_transactions(raw, intent, max_count=4)
        assert "raw_low" not in selected

    def test_all_raw_when_no_intents(self):
        validator = EscapeHatchValidator(min_quota=0.10)
        raw = [(f"raw_{i}", i) for i in range(5)]
        selected = validator.select_transactions(raw, [], max_count=3)
        assert len(selected) == 3
        assert all(s.startswith("raw_") for s in selected)
